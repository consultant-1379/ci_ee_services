...
import static com.ericsson.eniq.events.server.common.ApplicationConstants.CSV_PARAM;
import static com.ericsson.eniq.events.server.common.ApplicationConstants.DAY;
import static com.ericsson.eniq.events.server.common.ApplicationConstants.FIFTEEN_MINUTES;
...
import static com.ericsson.eniq.events.server.common.ApplicationConstants.REQUEST_URI;
import static com.ericsson.eniq.events.server.common.ApplicationConstants.TZ_OFFSET;
import static com.ericsson.eniq.events.server.logging.performance.ServicesPerformanceThreadLocalHolder.getContextInfo;
...
import static com.ericsson.eniq.events.server.logging.performance.ServicesPerformanceThreadLocalHolder.setRequestEndTime;
import static com.ericsson.eniq.events.server.logging.performance.ServicesPerformanceThreadLocalHolder.setRequestStartTime;
...
/**
 * The base class for all resource services in the services layer.
 * This class is responsible for controlling the flow through the services layer ie the order in which steps such
 * as logging, parameter checking, query generation and query execution are performed.
 * <p/>
 * Only resource/query specific logic should exist in the subclasses of this class.
 * 
 * @TODO update how tech packs are licensed (this was updated in a hurry for delivery to Smarttone)
 * All techpack licensing should happen in one place (currently called from here, in getAndRunQuery() and in
 * TechPackListFactory).  Also to consider - having some sort of Tech Pack object, which is intercepted by some
 * sort of licensing interceptor, which filters out the unlicensed tech packs
 *
 * @author EEMECOY
 */
public abstract class GenericService implements Service, GenericServiceInterface {
    ...
    public String runQuery(final String query, final String requestId,
        ...
        return getDataService().getGridData(requestId, query, queryParameters, getTimeColumnIndices(), tzOffset,
                loadBalancingPolicy);
    }

    /**
     * @param parameters          parameters from the resource layer
     * @param httpServletResponse response object (can be null, used when streaming csv response)
    ...
    private String getAndRunQuery(final MultivaluedMap<String, String> parameters,
        ...
        try {
            final RequiredParameters requiredParameters = new RequiredParameters(getStaticParameters(),
                    getRequiredParametersForQuery(), requiredToCheckValidParameterValue(parameters));
            final String errorMessage = parameterChecker.performValidityChecking(requiredParameters, parameters,
                    getApplicableTechPacks(parameters));
            if (StringUtils.isNotEmpty(errorMessage)) {
                return errorMessage;
            }
            final List<String> licensedTechPacks = techPackLicensingService
                    .getLicensedTechPacks(getApplicableTechPacks(parameters));
            if (licensedTechPacks.isEmpty()) {
                return getJSONErrorForNoLicensedTechPacksPresent(parameters);
            }
            final FormattedDateTimeRange formattedDateTimeRange = translateDateTimeParameters(parameters,
                    licensedTechPacks);
    ...
    private String logAndRunQuery(final HttpServletResponse httpServletResponse,
            final MultivaluedMap<String, String> parameters, final FormattedDateTimeRange formattedDateTimeRange,
            final String query) {
        ...
        if (mediaTypeHandler.isMediaTypeApplicationCSV(parameters.get(MEDIA_TYPE))) {
            streamDataAsCSV(parameters, parameters.getFirst(TZ_OFFSET), getTimeColumnIndices(), query,
                    httpServletResponse, queryParameters);
            return null;
        }
     ...
     */
    private void streamDataAsCSV(final MultivaluedMap<String, String> parameters, final String tzOffset,
            final List<Integer> timeColumnIndexes, final String query, final HttpServletResponse response,
            final Map<String, QueryParameter> queryParameters) {
        response.setContentType("application/csv");
        response.setHeader("Content-disposition", "attachment; filename=export.csv");
        try {
            this.streamingDataService.streamDataAsCsv(query, queryParameters, timeColumnIndexes, tzOffset,
    ...
    /**
     * Returns true if
     * 1) raw tables have not been found for the specified tech packs, and raw tables are required for the
     * aggregation queries
     * or
     * 2) raw tables have not been found for the specified tech packs, and this query is using the raw tables (eg its
    ...
    private Map<String, QueryParameter> getQueryParameters(final MultivaluedMap<String, String> requestParameters,
        ...
        queryParameters.putAll(this.queryUtils.getQueryParameters(requestParameters, formattedDateTimeRange,
                getTemplatePath()));
    ...
    protected String getQuery(final MultivaluedMap<String, String> parameters,
            final FormattedDateTimeRange formattedDateTimeRange, final TechPackList techPackList) {
        preQueryTracing(parameters);
        final QueryGeneratorParameters queryGeneratorParameters = constructQueryGeneratorParameters(
                formattedDateTimeRange, parameters, techPackList);
        return queryGenerator.getQuery(queryGeneratorParameters);
    }

    private TechPackList createTechPackList(final FormattedDateTimeRange formattedDateTimeRange,
        ...
        if (forceAggregationType() != null) {
            tpList = techPackListFactory.createTechPackListWithSpecifiedAggregation(
                    getApplicableTechPacks(requestParameters), formattedDateTimeRange, getAggregationView(type),
                    forceAggregationType());
        } else if (rawTableKeys == null && measurementTypes == null) {
            tpList = techPackListFactory.createTechPackList(getApplicableTechPacks(requestParameters),
                    formattedDateTimeRange, getAggregationView(type));
        } else if (rawTableKeys != null && measurementTypes == null) {
            tpList = techPackListFactory.createTechPackListWithKeys(getApplicableTechPacks(requestParameters),
                    rawTableKeys, formattedDateTimeRange, getAggregationView(type));
        } else if (rawTableKeys == null && measurementTypes != null) {
            tpList = techPackListFactory.createTechPackListWithMeasuermentType(
                    getApplicableTechPacks(requestParameters), measurementTypes, formattedDateTimeRange,
                    getAggregationView(type), getTableSuffixKey());
        }
        return tpList;
    }

    private QueryGeneratorParameters constructQueryGeneratorParameters(
            final FormattedDateTimeRange formattedDateTimeRange,
            final MultivaluedMap<String, String> requestParameters, final TechPackList techPackList) {
        final Map<String, Object> templateParam = getServiceSpecificTemplateParameters(requestParameters,
                formattedDateTimeRange, techPackList);
        ...
        return new QueryGeneratorParameters(getTemplatePath(), requestParameters, templateParam,
                formattedDateTimeRange, getDrillDownTypeForService(requestParameters), getMaxAllowableSize(),
                techPackList, getKPIList(), exclusiveTACHandler.queryIsExclusiveTacRelated(requestParameters),
                dataTieringHandler.useDataTieringView(formattedDateTimeRange, isDataTieredService(requestParameters),
                        techPackList.getTechPacks()));

    }

    /**
     * Default implementation of getKPIList() to reduce impact on existing services.
     * This parameter
     *
     * @return
     */
    ...
    private FormattedDateTimeRange translateDateTimeParameters(final MultivaluedMap<String, String> parameters,
            final List<String> licensedTechPacks) {
        final FormattedDateTimeRange timeRange = dateTimeHelper.translateDateTimeParameters(parameters,
                licensedTechPacks);

        if (dataTieringHandler.appplyLatencyForDataTiering(timeRange, isDataTieredService(parameters),
                licensedTechPacks, parameters)) {
    ...
    /**
     * Given a list of request parameters that should be injected directly into the query, construct a QueryParameter
     * object for each of these request parameters.
     *
     * @param requestParametersToIncludeInQuery
     *                   list of the request parameters that should be injected
     *                   directly into the SQL query with the same parameter name
     * @param parameters parameters provided by resource layer
     * @return map of query parameters and their values to be used in SQL query
     */
    protected Map<String, QueryParameter> mapRequestParametersDirectlyToQueryParameters(
            final List<String> requestParametersToIncludeInQuery, final MultivaluedMap<String, String> parameters) {
        ...
        for (final String requestParameter : requestParametersToIncludeInQuery) {
            final QueryParameter queryParameter = queryUtils.createQueryParameter(requestParameter,
                    parameters.getFirst(requestParameter));
     ...
     * @param requestParameters  parameters provided by resource layer
     */
    ...
    /**
     * This function is used to get the interval time for the template. 
     * (for example, Event Volume & Network Event Volume need a interval for result column time.)
     * @param dateTimeRange 
     * @return interval time 
     * when timerange = TR_1 & TR_2 return 1 min
     * when timerange = TR_3 return 15 mins
     * when timerange = TR_4 return 1440 mins (1day)
     */
    ...
    /**
     * @param parameterChecker the parameterChecker to set
     */
     ...
     *
     * @param queryGenerator The query generation logic
     */
     ...
     */
    public void setTechPackDescriptionMappingsService(
            final TechPackDescriptionMappingsService techPackDescriptionMappingsService) {
