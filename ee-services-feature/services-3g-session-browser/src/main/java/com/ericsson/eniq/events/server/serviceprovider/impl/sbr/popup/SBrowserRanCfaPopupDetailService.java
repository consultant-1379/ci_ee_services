...
import com.ericsson.eniq.events.ui.shared.model.sessionbrowser.popup.ICfaCallSetupFailurePopupDetails;
import com.ericsson.eniq.events.ui.shared.model.sessionbrowser.popup.ICfaSystemReleasePopupDetails;
...
@Stateless
@Local(Service.class)
public class SBrowserRanCfaPopupDetailService extends GenericService {

    private static final int INTERNAL_CALL_SETUP_FAIL_EVENT_ID = 456;

    private static final int INTERNAL_SYSTEM_RELEASE = 438;
    ...
    public String getTemplatePath() {
        return POPUP_DETAIL + PATH_SEPARATOR + RAN_CFA;
    }
    ...
    public String runQuery(final String query, final String requestId,
        ...
        if (eventId == INTERNAL_CALL_SETUP_FAIL_EVENT_ID) {
            transformer = new JsonResultSetTransformer(timeStampFrom, timeStampTo, tzOffset,
                    ICfaCallSetupFailurePopupDetails.class, new GenericDetailsPopupProcessor(
                            ICfaCallSetupFailurePopupDetails.class, tzOffset));
        } else if (eventId == INTERNAL_SYSTEM_RELEASE) {
            transformer = new JsonResultSetTransformer(timeStampFrom, timeStampTo, tzOffset,
                    ICfaSystemReleasePopupDetails.class, new GenericDetailsPopupProcessor(
                            ICfaSystemReleasePopupDetails.class, tzOffset));
        }
