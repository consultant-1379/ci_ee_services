...
public abstract class MssBaseResource extends BaseResource {
     ...
     */
    private FormattedDateTimeRange getFormattedDateTimeRangeWithMSSOffsets(
        ...
        final FormattedDateTimeRange dateTimeRange = DateTimeRange.getFormattedDateTimeRange(
                ...
                applicationConfigManager.getMSSTimeDelayOneMinuteData(),
                applicationConfigManager.getMSSTimeDelayFifteenMinuteData(),
     ...
     *            range
     * @param key
     *            to differentiate which RAW tables we need (e.g. error,
     *            success)
     * @param templateKey
     *            key passed to template eg rawAllErrTables
     *
     * @return the raw table names
     */

    protected List<String> getRAWTablesUsingQuery(final FormattedDateTimeRange newDateTimeRange, final String key,
        ...
        if (rawtables == null || rawtables.isEmpty()) {
            ...
            final String queryForLatestTable = this.templateUtils.getQueryFromTemplate(
                    getTemplate(getRawTables, requestParameters, drillType), templateParametersForRawTables);
        ...
        if (rawtables == null || rawtables.isEmpty()) {
            ...
            final String queryForAnyTable = this.templateUtils.getQueryFromTemplate(
                    getTemplate(getRawTables, requestParameters, drillType), templateParametersForRawTables);
    ...
    protected List<String> getTablesFromEngine(final FormattedDateTimeRange newDateTimeRange, final String key,
            final String templateKey) throws ParseException {
        List<String> rawtables = new ArrayList<String>();
        if (isMSSTable(templateKey)) {
            rawtables = this.rmiEngineUtils
                    .getTableNamesForMSS(
                            new Timestamp(DateTimeRange.getUTCDateTimeWithSeconds(newDateTimeRange.getStartDateTime())
                                    .getTime()),
                            new Timestamp(DateTimeRange.getUTCDateTimeWithSeconds(newDateTimeRange.getEndDateTime())
                                    .getTime()), key);
        }
        return rawtables;
    }
    ...
    /**
     * return true if template key for table name eg rawMssErrTables contains
     * the text mss (not case sensitive)
     *
     * @param templateKey
     *            key passed to template eg rawAllErrTables
     ...
     * query the time range views to determine which raw tables are applicable
     * for this time range. Depends on the sub resource sub class populating the
     * aggregationViews map with the correct information for the query/resource
     *
     * @param type
     *            one of TYPE_APN, TYPE_SGSN etc
     * @param dateTimeRange
     * @param listOfTechPacks
     *            list of the tech packs that should be considered for this
     *            request ie EVENT_E_SGEH, EVENT_E_LTE or both
     *
     */
    @Override
    protected TechPackTables getRawTables(final String type, final FormattedDateTimeRange dateTimeRange,
        ...
        for (final String techPackName : listOfTechPacks) {
            if (isTechPackApplicableForType(type, techPackName) && isTechPackInstalled(techPackName)) {
                ...
                final List<String> errTables = getRAWTables(dateTimeRange, KEY_TYPE_ERR,
                        getTemplateKeyMatchingTechPack(techPackName));
                if (errTables.size() > 0) {
                    techPack.setErrRawTables(errTables);
                }
                final List<String> droppedTables = getRAWTables(dateTimeRange, KEY_TYPE_DROP_CALL,
                        getTemplateKeyMatchingTechPack(techPackName));
                if (droppedTables.size() > 0) {
                    techPack.setErrRawTables(KEY_TYPE_DROP_CALL, droppedTables);
                }
                final List<String> sucTables = getRAWTables(dateTimeRange, KEY_TYPE_SUC,
                        getTemplateKeyMatchingTechPack(techPackName));
                if (sucTables.size() > 0) {
                ...
                final List<String> smsSucTables = getRAWTables(dateTimeRange, KEY_TYPE_SMS_SUC,
                        getTemplateKeyMatchingTechPack(techPackName));
                if (smsSucTables.size() > 0) {
                    techPack.setSucRawTables(KEY_TYPE_SMS_SUC, smsSucTables);
                }
                final List<String> smsErrTables = getRAWTables(dateTimeRange, KEY_TYPE_SMS_ERR,
                        getTemplateKeyMatchingTechPack(techPackName));
    ...
    /**
     * This function is used for getting the raw tables which could replace a
     * raw view for a particular time range The function first uses the RMI
     ...
     *            success)
     * @param templateKey
     *            key passed to template eg rawMssTables
     * @return the raw table names
     */
    @Override
    protected List<String> getRAWTables(final FormattedDateTimeRange newDateTimeRange, final String key,
        ...
        try {
            ...
            ServicesLogger.warn(BaseResource.class.getName(), "getRAWTables",
                    "Exception thrown while trying to parse the date in order to get the table names from the engine cache. "
                            + e.getMessage());
        }
        if (rawtables == null || rawtables.isEmpty()) {
     ...
     * @return the formatted date time range
     */
    protected FormattedDateTimeRange getAndCheckFormattedDateTimeRange(
            final MultivaluedMap<String, String> requestParameters) {
        final FormattedDateTimeRange timerange = DateTimeRange.getFormattedDateTimeRangeWithOutOffset(
     ...
     * streaming the csv data into the response using hashId.
     *
     * @param requestParameters
     *            the request parameters
     * @param tzOffset
     ...
     */
    protected void streamDataAsCSV(final MultivaluedMap<String, String> requestParameters, final String tzOffset,
            final List<Integer> timeColumnIndexes, final List<String> queries,
            final FormattedDateTimeRange newDateTimeRange) {
        response.setContentType("application/csv");
        response.setHeader("Content-disposition", "attachment; filename=export.csv");
        try {
            this.streamingDataService.streamDataAsCsv(queries,
                    this.queryUtils.mapRequestParametersForHashId(requestParameters, newDateTimeRange),
                    timeColumnIndexes, tzOffset, getLoadBalancingPolicy(requestParameters), response.getOutputStream());
        } catch (final IOException e) {
            ServicesLogger.error(getClass().getName(), "streamDataAsCSV", e);
    ...
    /**
     * Get the time range for the request - the method
     * getEventDataSourceTypeForGridAndNonTimeSeriesChartWithoutOneMinAggregated
     * () in QueryUtils is used to determine which time range applies for the
     * given dateTimeRange
     *
     * @param dateTimeRange
     * @return
    ...
    protected TechPackTables getAggregationTables(final String type, final String timerange,
        ...
        if (aggrTableInfo instanceof MssAggregationTableInfo) {
            ...
            return getAggregationTables(mssAggrTableInfo.getAggregationViewVoice(),
                    mssAggrTableInfo.getAggregationViewSMS(), mssAggrTableInfo.getAggregationViewLosService(), time,
     ...
     *
     * @param aggregationViewForVoice
     *            aggregation view name for voice
     * @param aggregationViewForSMS
     *            aggregation view name for sms
     * @param aggregationViewForLocService
     *            aggregation view name for loc_service
     * @param time
     * @param type
     * @param listOfTechPacks
     * @return
     */
    private TechPackTables getAggregationTables(final String aggregationViewForVoice,
            final String aggregationViewForSMS, final String aggregationViewForLocService, final String time,
        ...
        for (final String techPackName : listOfTechPacks) {
            if (isTechPackApplicableForType(type, techPackName) && isTechPackInstalled(techPackName)) {
                final TechPack techPack = new TechPack(techPackName, TableType.AGGREGATION,
                        getMatchingDIMTechpack(techPackName));
                final String errVoiceTable = techPackName + UNDERSCORE + VOICE + UNDERSCORE + aggregationViewForVoice
                        + UNDERSCORE + KEY_TYPE_ERR + time;
                techPack.setErrAggregationView(errVoiceTable);

                final String dropVoiceTable = techPackName + UNDERSCORE + VOICE + UNDERSCORE + aggregationViewForVoice
                        + UNDERSCORE + KEY_TYPE_DROP_CALL + time;
                ...
                if (aggregationViewForSMS != null) {
                    final String errSMSTable = techPackName + UNDERSCORE + SMS + UNDERSCORE + aggregationViewForSMS
                            + UNDERSCORE + KEY_TYPE_ERR + time;
                    techPack.setErrAggregationView(KEY_TYPE_SMS_ERR, errSMSTable);

                    final String sucSMSTable = techPackName + UNDERSCORE + SMS + UNDERSCORE + aggregationViewForSMS
                            + UNDERSCORE + KEY_TYPE_SUC + time;
                    techPack.setSucAggregationView(KEY_TYPE_SMS_SUC, sucSMSTable);
                }
                if (aggregationViewForLocService != null) {
                    final String errLocServiceTable = techPackName + UNDERSCORE + LOC_SERVICE + UNDERSCORE
                            + aggregationViewForLocService + UNDERSCORE + KEY_TYPE_ERR + time;
                    techPack.setErrAggregationView(KEY_TYPE_LOC_SERVICE_ERR, errLocServiceTable);

                    final String sucLocServiceTable = techPackName + UNDERSCORE + LOC_SERVICE + UNDERSCORE
                            + aggregationViewForLocService + UNDERSCORE + KEY_TYPE_SUC + time;
    ...
    /**
     * Update the type for MSS. Add MSS in front of the original type
     * parameters.
     *
     * @param type
     * @param requestParameters
    ...
    // type as its type
    protected String getTemplateForMss(final String pathName, final MultivaluedMap<String, String> requestParameters,
        ...
        final String[] keys = { pathName, String.valueOf(type), String.valueOf(isGroup), String.valueOf(drillType),
                String.valueOf(hasEventID), String.valueOf(key), String.valueOf(returnAggregateViewType(timerange)) };
        final MultiKey multiKey = new MultiKey(keys);
        return this.templateMappingEngine.getTemplate(multiKey);
    }

    /**
     * This method is used to determine the time column indexes based on the
     * eventID
     *
    ...
    /**
     * This method is used to determine whether to exclude events containing TAC
     * that are part of exclusive TAC group
     *
     * @param requestParameters
     *            url parameters
