...
public class MssEventAnalysisResource extends MssBaseResource {
    ...
    protected boolean isValidValue(final MultivaluedMap<String, String> requestParameters) {
        if (requestParameters.containsKey(NODE_PARAM) || requestParameters.containsKey(GROUP_NAME_PARAM)
                || requestParameters.containsKey(IMSI_PARAM) || requestParameters.containsKey(MSISDN_PARAM)) {
    ...
    private String getGridResults(final String requestId, final MultivaluedMap<String, String> requestParameters)
        ...
        if (TYPE_IMSI.equals(type) || TYPE_MSISDN.equals(type)) {
            ...
            dateTimeRange = getAndCheckFormattedDateTimeRangeForDailyAggregation(requestParameters,
                    TechPackData.completeTechPackListForMss);
        }
        ...
        return this.dataService.getGridDataWithAppendedRows(requestId, queries,
                this.queryUtils.mapRequestParametersForHashId(requestParameters, newDateTimeRange), timeColumnIndexes,
