...
import static com.ericsson.eniq.events.server.common.ApplicationConstants.HFA_HSDSCH;
import static com.ericsson.eniq.events.server.common.ApplicationConstants.HFA_IFHO;
import static com.ericsson.eniq.events.server.common.ApplicationConstants.HFA_IRAT;
...
import static com.ericsson.eniq.events.server.common.ApplicationConstants.WCDMA_HFA_EVENT_SUMMARY;
...
@Stateless
@Local(Service.class)
public class RNCHandoverFailureEventSummaryService extends GenericService {
    ...
    public String getTemplatePath() {
        return WCDMA_HFA_EVENT_SUMMARY + "/" + RNC;
    }
    ...
    public Map<String, QueryParameter> getServiceSpecificQueryParameters(
        ...
        Long hashedIdForController = null;
        if (!requestParameters.containsKey(GROUP_NAME_PARAM) && !requestParameters.containsKey(NODE_PARAM)) {
            ...
            hashedIdForController = getQueryUtils().createHashIDForController(_3G, controller, vendor);
        } else if (requestParameters.containsKey(NODE_PARAM)) {
            final String node = requestParameters.getFirst(NODE_PARAM);
            if (node != null) {
                final String[] allData = node.split(DELIMITER);
                hashedIdForController = getQueryUtils().createHashIDForController(_3G, allData[0],
                        allData[1]);
            }
        }
