...
import com.ericsson.eniq.events.server.serviceprovider.impl.wcdma.hfa.ranking.SubscriberIFHOFailureRankingService;
...
import static com.ericsson.eniq.events.server.test.temptables.TempTableNames.TEMP_EVENT_E_RAN_HFA_IFHO_IMSI_ERR_15MIN;
import static com.ericsson.eniq.events.server.test.temptables.TempTableNames.TEMP_EVENT_E_RAN_HFA_IFHO_IMSI_ERR_DAY;
 ...
 *
 */
public class IMSIIFHOHFARSAggTest extends
    ...
    private SubscriberIFHOFailureRankingService subscriberIFHOFailureRankingService;

    @Before
    public void onSetUp() {
        subscriberIFHOFailureRankingService = new SubscriberIFHOFailureRankingService();
        attachDependencies(subscriberIFHOFailureRankingService);
    }

    @Test
    public void testGetRankingData_IMSI_HFA_15MINS() throws Exception {

        createAndPopulateAggTable(TEMP_EVENT_E_RAN_HFA_IFHO_IMSI_ERR_15MIN, DateTimeUtilities.getDateTimeMinus48Hours());

        final MultivaluedMap<String, String> map = new MultivaluedMapImpl();
        map.putSingle(DISPLAY_PARAM, "grid");
        map.putSingle(TIME_QUERY_PARAM, "10000");
        map.putSingle(TZ_OFFSET, "+0100");
        map.putSingle(MAX_ROWS, "10");
        map.putSingle(CATEGORY_ID_PARAM, WCDMA_HFA_IFHO_CATEGORY_ID);

        final String json = runQuery(subscriberIFHOFailureRankingService, map);
    ...
    public void testGetRankingData_IMSI_HFA_DAY() throws Exception {
        ...
        createAndPopulateAggTable(TEMP_EVENT_E_RAN_HFA_IFHO_IMSI_ERR_DAY, DateTimeUtilities.getDateTimeMinus48Hours());
        ...
        new DummyUriInfoImpl(map, "WCDMA_HANDOVER_FAILURE_RANKING_ANALYSIS/IMSI", "");
    ...
    private void createAndPopulateAggTable(final String tempTableName, final String dateTime) throws SQLException {
        ...
        try {
            ...
            sqlExecutor.executeUpdate("insert into " + tempTableName + " values(" + TEST_IMSI_0 + "," + "2" + ","
                    + "230" + ",'" + dateTime + "')");
            sqlExecutor.executeUpdate("insert into " + tempTableName + " values(" + TEST_IMSI_1 + "," + "2" + ","
                    + "23" + ",'" + dateTime + "')");
            sqlExecutor.executeUpdate("insert into " + tempTableName + " values(" + TEST_IMSI_2 + "," + "2" + ","
                    + "56" + ",'" + dateTime + "')");
            sqlExecutor.executeUpdate("insert into " + tempTableName + " values(" + TEST_IMSI_3 + "," + "2" + ","
                    + "60" + ",'" + dateTime + "')");
        } finally {
            if (sqlExecutor != null) {
                sqlExecutor.close();
