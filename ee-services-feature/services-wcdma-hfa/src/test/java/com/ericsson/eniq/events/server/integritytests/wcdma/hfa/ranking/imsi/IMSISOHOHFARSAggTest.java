...
import com.ericsson.eniq.events.server.serviceprovider.impl.wcdma.hfa.ranking.SubscriberSOHOFailureRankingService;
 ...
 *
 */
public class IMSISOHOHFARSAggTest extends
    ...
    private SubscriberSOHOFailureRankingService subscriberSOHOFailureRankingService;

    @Before
    public void onSetUp() {
        subscriberSOHOFailureRankingService = new SubscriberSOHOFailureRankingService();
        attachDependencies(subscriberSOHOFailureRankingService);
    }

    @Test
    public void testGetRankingData_IMSI_HFA_15MINS() throws Exception {

        createAndPopulateAggTable("#EVENT_E_RAN_HFA_SOHO_IMSI_ERR_15MIN", DateTimeUtilities.getDateTimeMinus48Hours());
        ...
        map.putSingle(CATEGORY_ID_PARAM, WCDMA_HFA_SOHO_CATEGORY_ID);

        final String json = runQuery(subscriberSOHOFailureRankingService, map);
    ...
    public void testGetRankingData_IMSI_HFA_DAY() throws Exception {
        ...
        createAndPopulateAggTable("#EVENT_E_RAN_HFA_SOHO_IMSI_ERR_DAY", DateTimeUtilities.getDateTimeMinus48Hours());
    ...
    private void createAndPopulateAggTable(final String tempTableName, final String dateTime) throws SQLException {
        ...
        try {
            ...
            sqlExecutor.executeUpdate("insert into " + tempTableName + " values(" + TEST_IMSI_0 + "," + "0" + ","
                    + "230" + ",'" + dateTime + "')");
           sqlExecutor.executeUpdate("insert into " + tempTableName + " values(" + TEST_IMSI_1 + "," + "0" + ","
                    + "23" + ",'" + dateTime + "')");
            sqlExecutor.executeUpdate("insert into " + tempTableName + " values(" + TEST_IMSI_2 + "," + "0" + ","
                    + "56" + ",'" + dateTime + "')");
            sqlExecutor.executeUpdate("insert into " + tempTableName + " values(" + TEST_IMSI_3 + "," + "0" + ","
                    + "60" + ",'" + dateTime + "')");
        } finally {
            if (sqlExecutor != null) {
                sqlExecutor.close();
