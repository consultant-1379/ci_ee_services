...
import static com.ericsson.eniq.events.server.common.ApplicationConstants.WCDMA_HFA_IFHO_CATEGORY_ID;
...
import com.ericsson.eniq.events.server.serviceprovider.impl.wcdma.hfa.terminalanalysis.TerminalAnalysisIFHOFailureService;
 ...
 *
 */
public class TAIFHOFailureServiceRawTest extends
        BaseDataIntegrityTest<HandoverFailureTerminalAnalysisResult> {

    private TerminalAnalysisIFHOFailureService service;

    private final int numFailuresForSampleTac = 2;

    @Before
    public void setUp() throws Exception {
        service = new TerminalAnalysisIFHOFailureService();
    ...
    @Test
    public void getTerminalAnalysisIFHOFailure() throws Exception {
        ...
        //validate result
        //validateAgainstGridDefinition(result, "TERMINAL_ANALYSIS_RAN_WCDMA_HFA_IFHO");
        ...
        final HandoverFailureTerminalAnalysisResult mostIFHOFailure = results.get(0);
        assertThat(mostIFHOFailure.getRank(), is(1));
        assertThat(mostIFHOFailure.getManufacturer(), is(MANUFACTURER_FOR_SAMPLE_TAC));
        assertThat(mostIFHOFailure.getModel(), is(MARKETING_NAME_FOR_SAMPLE_TAC));
        assertThat(mostIFHOFailure.getTac(), is(SAMPLE_TAC));
        assertThat(mostIFHOFailure.getFailures(), is(numFailuresForSampleTac));
        assertThat(mostIFHOFailure.getCategory(), is(Integer.parseInt(WCDMA_HFA_IFHO_CATEGORY_ID)));
    }
    ...
    private void insertRowData() throws SQLException {
        for (int i = 0; i < numFailuresForSampleTac; i++) {
            ...
            values.put(CATEGORY_ID, Integer.parseInt(WCDMA_HFA_IFHO_CATEGORY_ID));
