 ...
 * @since  2011
 */
public class ControllerGroupCallFailureEventSummaryRawTest extends
    ...
    private void insertEventData() throws SQLException {
        ...
        //Use of GSM related constant to avoid adding new constants
        insertRowIntoEventTable(TEST_VALUE_GSM_HIER3_ID_BSC1, CALL_DROP_EVENT_ID, SAMPLE_TAC, timestamp);
        insertRowIntoEventTable(TEST_VALUE_GSM_HIER3_ID_BSC1, CALL_DROP_EVENT_ID, SAMPLE_TAC, timestamp);

        insertRowIntoEventTable(TEST_VALUE_GSM_HIER3_ID_BSC1, CALL_SETUP_FAILURE_EVENT_ID, SAMPLE_TAC, timestamp);
        insertRowIntoEventTable(TEST_VALUE_GSM_HIER3_ID_BSC1, CALL_SETUP_FAILURE_EVENT_ID, SAMPLE_TAC, timestamp);

        insertRowIntoEventTable(TEST_VALUE_GSM_HIER3_ID_BSC2, CALL_DROP_EVENT_ID, SAMPLE_TAC, timestamp);
        insertRowIntoEventTable(TEST_VALUE_GSM_HIER3_ID_BSC2, CALL_DROP_EVENT_ID, SAMPLE_EXCLUSIVE_TAC, timestamp);

        insertRowIntoEventTable(TEST_VALUE_GSM_HIER3_ID_BSC2, CALL_SETUP_FAILURE_EVENT_ID, SAMPLE_TAC, timestamp);
        insertRowIntoEventTable(TEST_VALUE_GSM_HIER3_ID_BSC2, CALL_SETUP_FAILURE_EVENT_ID, SAMPLE_EXCLUSIVE_TAC,
                timestamp);
    }

    private void insertRowIntoEventTable(final long hier3Id, final String eventId, final long tac,
            final String timestamp) throws SQLException {
        final Map<String, Object> valuesForEventTable = new HashMap<String, Object>();
        valuesForEventTable.put(HIER3_ID, hier3Id);
        valuesForEventTable.put(TAC, tac);
        valuesForEventTable.put(EVENT_ID, eventId);
        valuesForEventTable.put(DATETIME_ID, timestamp);
        valuesForEventTable.put(IMSI_PARAM, SAMPLE_IMSI);
    ...
    private void verifyResult(final String json) throws Exception {
        ...
        assertThat(resultForCallDropsEvent.getNumOfFailures(), is("3"));
        ...
        assertThat(resultForCallSetupFailureEvent.getNumOfFailures(), is("3"));
