...
import com.ericsson.eniq.events.server.test.queryresults.NetworkEventAnalysisResult;
 ...
 * 
 */
public class NetworkEventAnalysisControllerAggTest extends BaseDataIntegrityTest<NetworkEventAnalysisResult> {

    private NetworkEventAnalysisService networkEventAnalysisService;

    private static final String TEMP_EVENT_E_RAN_CFA_HIER3_CELL_ID_EVENTID_ERR_15MIN = "#EVENT_E_RAN_CFA_HIER3_CELL_ID_EVENTID_ERR_15MIN";

    private static final String TEMP_EVENT_E_RAN_CFA_HIER3_EVENTID_ERR_15MIN = "#EVENT_E_RAN_CFA_HIER3_EVENTID_ERR_15MIN";

    private static final String TEMP_EVENT_E_RAN_CFA_HIER3_ALLCALLS_15MIN = "#EVENT_E_RAN_CFA_HIER3_ALLCALLS_15MIN";
    ...
    @Test
    public void testNetworkRncData_TotalRab_15MIN() throws Exception {
        final MultivaluedMap<String, String> requestParameters = new MultivaluedMapImpl();
        requestParameters.putSingle(TIME_FROM_QUERY_PARAM, "2354");
        requestParameters.putSingle(TIME_TO_QUERY_PARAM, "0602");
        ...
        requestParameters.putSingle(NODE_PARAM, RNC01_ALTERNATIVE_FDN + COMMA + ERICSSON + COMMA + _3G);
        requestParameters.putSingle(TYPE_PARAM, "BSC");

        final String json = runQuery(networkEventAnalysisService, requestParameters);
        System.out.println(json);

        final List<NetworkEventAnalysisResult> results = getTranslator().translateResult(json,
                NetworkEventAnalysisResult.class);
        assertThat(results.size(), is(1));

        final NetworkEventAnalysisResult resultForCallDropsEvent = results.get(0);
    ...
    private void populateData() throws SQLException {
        ...
        insertRow(TEMP_EVENT_E_RAN_CFA_HIER3_EVENTID_ERR_15MIN, new HashMap<String, Object>() {
            {
                put("HIER3_ID", hashedIdForRNC);
                put("EVENT_ID", 438);
                put("CS_RAB_FAIL_CNT", 1);
        ...
        insertRow(TEMP_EVENT_E_RAN_CFA_HIER3_CELL_ID_EVENTID_ERR_15MIN, new HashMap<String, Object>() {
            {
                put("HIER3_CELL_ID", hashedIdForCell);
                put("EVENT_ID", 456);
                put("CS_RAB_FAIL_CNT", 1);
        ...
        insertRow(TEMP_EVENT_E_RAN_CFA_HIER3_ALLCALLS_15MIN, new HashMap<String, Object>() {
            {
                put("HIER3_ID", hashedIdForRNC);
                put("CALLS_PS_ALL", 1);
                put("CALLS_CS_ALL", 1);
    ...
    private void createCfaDataTableSchema() throws SQLException {
        ...
        final List<ColumnDetails> hier321ErrTableSchema = new ArrayList<ColumnDetails>() {
            {
                add(new ColumnDetails("HIER3_ID", "unsigned bigint", Nullable.CAN_BE_NULL));
                add(new ColumnDetails("HIER3_CELL_ID", "unsigned bigint", Nullable.CAN_BE_NULL));
                add(new ColumnDetails("EVENT_ID", "smallint", Nullable.CAN_BE_NULL));
        ...
        final List<ColumnDetails> hier3AllCallFifteenTableSchema = new ArrayList<ColumnDetails>() {
            {
                add(new ColumnDetails("HIER3_ID", "unsigned bigint", Nullable.CAN_BE_NULL));
        ...
        createTemporaryTable(TEMP_EVENT_E_RAN_CFA_HIER3_EVENTID_ERR_15MIN, hier3ErrTableSchema);
        createTemporaryTable(TEMP_EVENT_E_RAN_CFA_HIER3_CELL_ID_EVENTID_ERR_15MIN, hier321ErrTableSchema);
        createTemporaryTable(TEMP_EVENT_E_RAN_CFA_HIER3_ALLCALLS_15MIN, hier3AllCallFifteenTableSchema);
