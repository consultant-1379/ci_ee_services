...
import static com.ericsson.eniq.events.server.common.EventIDConstants.CALL_SETUP_FAILURE_EVENT_ID_AS_INTEGER;
...
import com.ericsson.eniq.events.server.serviceprovider.impl.wcdma.callfailure.eventanalysis.NetworkDetailedEventAnalysisByFilterService;
import com.ericsson.eniq.events.server.test.queryresults.FailedEventAnalysisByFilterResult;
 ...
 */
@Ignore
public class ControllerDetailedEventAnalysisByFilterTest extends
        BaseDataIntegrityTest<FailedEventAnalysisByFilterResult> {

    private NetworkDetailedEventAnalysisByFilterService service;

    private String expectedTime;

    private final String HASH_CELL_ID = "1792322973440499887";

    private final String HASH_CONTROLLER_ID = "4894708643454430832";

    String groupName = "sampleGroup";

    @Before
    public void setup() throws Exception {
        service = new NetworkDetailedEventAnalysisByFilterService();
        ...
        insertresult(SAMPLE_TAC, CALL_DROP_EVENT_ID_AS_INTEGER);
        insertresult(SAMPLE_TAC_2, CALL_DROP_EVENT_ID_AS_INTEGER);
        insertresult(SAMPLE_TAC, CALL_SETUP_FAILURE_EVENT_ID_AS_INTEGER);
        insertresult(SAMPLE_TAC_2, CALL_SETUP_FAILURE_EVENT_ID_AS_INTEGER);
        insertresult(SAMPLE_TAC_2, CALL_SETUP_FAILURE_EVENT_ID_AS_INTEGER);
        //This result should appear in the result set as it is an Exclusive Tac
        insertresult(SAMPLE_EXCLUSIVE_TAC, CALL_DROP_EVENT_ID_AS_INTEGER);
        insertresult(SAMPLE_EXCLUSIVE_TAC, CALL_SETUP_FAILURE_EVENT_ID_AS_INTEGER);
        insertDataIntoGroupTables();
    }

    public void setCommonRequestParameters(final MultivaluedMap<String, String> requestParameters) {
    ...
    @Test
    public void testFiveMinuteQueryForControllerByRabType() throws URISyntaxException, Exception {
        final MultivaluedMap<String, String> requestParameters = new MultivaluedMapImpl();
        setCommonRequestParameters(requestParameters);
        requestParameters.add(FILTER_TYPE, ApplicationConstants.RAB_TYPE);
        requestParameters.add(FILTER_VALUE, "1");

        requestParameters.add(HIER3_ID, "4894708643454430832");
        requestParameters.add(TYPE_PARAM, BSC);
        requestParameters.add("eventID", "438");
        requestParameters.add(CATEGORY_ID_PARAM, "0");

        final String result = runQuery(service, requestParameters);
        verifyResultForRabType(result);
    }

    //    @Test
    //    public void testFiveMinuteQueryForCellByRabType() throws URISyntaxException, Exception {
    //        final MultivaluedMap<String, String> requestParameters = new MultivaluedMapImpl();
    //        setCommonRequestParameters(requestParameters);
    //        requestParameters.add(FILTER_TYPE, RAB_TYPE);
    //        requestParameters.add(FILTER_VALUE, "1");
    //
    //        requestParameters.add(HIER3_CELL_ID, HASH_CELL_ID);
    //        requestParameters.add("eventID", "438");
    ...
    private void verifyResultForRabType(final String json) throws Exception {

        assertJSONSucceeds(json);
        final List<FailedEventAnalysisByFilterResult> results = getTranslator().translateResult(json,
                FailedEventAnalysisByFilterResult.class);
    ...
    @Test
    public void testFiveMinuteQueryForControllerByDisconnection() throws URISyntaxException, Exception {
        final MultivaluedMap<String, String> requestParameters = new MultivaluedMapImpl();
        setCommonRequestParameters(requestParameters);
        //        requestParameters.add(FILTER_TYPE, DISCONNECTION_CODE);
        //        requestParameters.add(FILTER_VALUE, "2");
    ...
    //    @Test
    //    public void testFiveMinuteQueryForCellByDisconnection() throws URISyntaxException, Exception {
    //        final MultivaluedMap<String, String> requestParameters = new MultivaluedMapImpl();
    //        setCommonRequestParameters(requestParameters);
    //        requestParameters.add(FILTER_TYPE, DISCONNECTION_CODE);
    //        requestParameters.add(FILTER_VALUE, "2");
    ...
    //        requestParameters.add(TYPE_PARAM, CELL);
    //        final String result = runQuery(service, requestParameters);
    //        verifyResultForDisconnection(result);
    //    }

    //    private void verifyResultForDisconnection(final String json) throws Exception {
    //
    //        assertJSONSucceeds(json);
    //        final List<FailedEventAnalysisByFilterResult> results = getTranslator().translateResult(json,
    //                FailedEventAnalysisByFilterResult.class);
     ...
     * @throws Exception
     */
    private void verifyResult(final List<FailedEventAnalysisByFilterResult> results) throws Exception {

        for (final FailedEventAnalysisByFilterResult result : results) {
            ...
            if (result.getTac() == SAMPLE_TAC) {
                ...
                assertThat(result.getTerminalMake(), is(MANUFACTURER_FOR_SAMPLE_TAC_2));
            ...
            assertThat(result.getC_id_serv_hsdsch_cell(), is("Cell"));
            assertThat(result.getCrnc_id_serv_hsdsch_cell(), is(RNC01_ALTERNATIVE_FDN));
     ...
     * @throws Exception
     */
    private void insertresult(final int tac, final int eventId) throws Exception {
        final Map<String, Object> dataForEventTable = new HashMap<String, Object>();
        expectedTime = DateTimeUtilities.getDateTimeMinus25Minutes();
        //Populate default lokup values
        dataForEventTable.putAll(getDefaultLookupValues());

        dataForEventTable.put(EVENT_TIME, expectedTime);
        dataForEventTable.put(IMSI, SAMPLE_IMSI);
        dataForEventTable.put(TAC, tac);
        dataForEventTable.put(EVENT_ID, eventId);
        dataForEventTable.put(DATETIME_ID, expectedTime);
        dataForEventTable.put(HIER3_CELL_ID, HASH_CELL_ID);
        dataForEventTable.put(HIER3_ID, HASH_CONTROLLER_ID);
        ...
        dataForEventTable.put(C_ID_SERV_HSDSCH_CELL, 1);
        ...
        dataForEventTable.put(RAN_DISCONNECTION_CODE, 2);
    ...
    private Map<String, Object> getDefaultLookupValues() {
        ...
        lookupValues.put(HIER3_ID, HASH_CONTROLLER_ID);
    ...
    private void populateTopologyTable() throws Exception {
        ...
        rowData.put(HIERARCHY_1, "");
        rowData.put(HIER3_CELL_ID_AGG, HASH_CELL_ID);
        rowData.put(VENDOR, ERICSSON);
        rowData.put(RAT, RAT_INTEGER_VALUE_FOR_3G);
        rowData.put(CELL_ID, "Cell");
        rowData.put(CID, 1);
        rowData.put(HIER3_ID, HASH_CONTROLLER_ID);
     ...
     *
     */
    private void insertDataIntoGroupTables() throws Exception {
        final Map<String, Object> valuesForTable = new HashMap<String, Object>();
        //Populate Tac Group table with Exclusive Tac
