...
import com.ericsson.eniq.events.server.test.queryresults.WCDMACallFailureNetworkControllerGroupDEAResult;
 ...
 * @since 2011
 */
public class ControllerGroupCallFailureDetailedEventAnalysisTest extends
        BaseDataIntegrityTest<WCDMACallFailureNetworkControllerGroupDEAResult> {

    private NetworkDetailedEventAnalysisService detailedEventService;
    ...
    public void onSetUp() throws Exception {
        detailedEventService = new NetworkDetailedEventAnalysisService();
        attachDependencies(detailedEventService);
        ReplaceTablesWithTempTablesTemplateUtils.addTableNameToReplace(DIM_E_SGEH_TAC, TEMP_DIM_E_SGEH_TAC);

        createAndPopulateLookupTables();
        createAndPopulate_DIM_E_SGEH_HIER321_CELL();
        createAndPopulateGroupTables();

        createTemporaryEventTable();
    ...
    public void testFiveMinuteQuery() throws URISyntaxException, Exception {
        ...
        requestParameters.putSingle(MAX_ROWS, "30");
        requestParameters.putSingle(TYPE_PARAM, TYPE_BSC);
        final String result = runQuery(detailedEventService, requestParameters);
        verifyResult(result);
    }
    ...
    private void verifyResult(final String json) throws Exception {
        ...
        validateAgainstGridDefinition(json, "NETWORK_DETAILED_EVENT_ANALYSIS_WCDMA_CFA_BSC_GROUP_CALL_DROPS");
        validateAgainstGridDefinition(json, "NETWORK_DETAILED_EVENT_ANALYSIS_WCDMA_CFA_BSC_GROUP_CALL_SETUP");

        final List<WCDMACallFailureNetworkControllerGroupDEAResult> results = getTranslator().translateResult(json,
                WCDMACallFailureNetworkControllerGroupDEAResult.class);

        assertThat(results.size(), is(2));
        for (final WCDMACallFailureNetworkControllerGroupDEAResult result : results) {
    ...
    private void createAndPopulate_DIM_E_SGEH_HIER321_CELL() throws Exception {
        ...
        rowData.put(HIER3_CELL_ID, HASH_ID);
    ...
    private void createAndPopulateGroupTables() throws Exception {
        ...
        valuesForTable.put(HIER3_ID, HASH_ID);
    ...
    /**
     * Create the raw event table
     *
     * @throws Exception
     */
    private void createTemporaryEventTable() throws Exception {
    ...
    private Map<String, Object> getDefaultLookupValues() {
        ...
        lookupValues.put(HIER3_CELL_ID, HASH_ID);
