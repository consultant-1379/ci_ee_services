 ...
 *
 */
public class CallFailureCountryRoamingAnalysisServiceRawTest extends
        BaseDataIntegrityTest<CallFailureCountryRoamingAnalysisQueryResult> {

    private CallFailureCountryRoamingAnalysisService roamingAnalysisService;

    @Before
    public void setup() throws Exception {
        roamingAnalysisService = new CallFailureCountryRoamingAnalysisService();
    ...
    private void insertRawData() throws SQLException {
        ...
        insertRowIntoRawTable(SAMPLE_IMSI, MCC_FOR_ARGENTINA, 1, timestamp);
        insertRowIntoRawTable(SAMPLE_IMSI, MCC_FOR_ARGENTINA, 1, timestamp);
        insertRowIntoRawTable(SAMPLE_IMSI_2, MCC_FOR_ARGENTINA, 1, timestamp);

        insertRowIntoRawTable(SAMPLE_IMSI, MCC_FOR_AFGHANISTAN, 1, timestamp);
        insertRowIntoRawTable(SAMPLE_IMSI, MCC_FOR_AFGHANISTAN, 1, timestamp);
        ...
        insertRowIntoRawTable(SAMPLE_IMSI, null, 0, timestamp);

    }

    private void insertRowIntoRawTable(final long imsi, final String mcc, final int roamingValue, final String timestamp)
            throws SQLException {
    ...
    @Test
    public void testGetCountryRoamingData_30Minutes() throws Exception {
    ...
    private void validateResult(final String json) throws Exception {
        ...
        final CallFailureCountryRoamingAnalysisQueryResult countryWithMostRoamingErrors = results.get(0);
        ...
        final CallFailureCountryRoamingAnalysisQueryResult countryWithSecondMostRoamingErrors = results.get(1);
