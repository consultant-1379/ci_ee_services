...
import com.ericsson.eniq.events.server.serviceprovider.impl.wcdma.callfailure.roaminganalysis.CallFailureCountryDrillDetailRoamingService;
 ...
 *
 */
public class CallFailureCountryDrillDetailRoamingServiceRawTest extends
        BaseDataIntegrityTest<CallFailurCountryDrilDetaillRoamingQueryResult> {

    private CallFailureCountryDrillDetailRoamingService callFailureCountryDrillDetailRoamingService;

    private static final String SAMPLE_MNC = "010";

    @Before
    public void setup() throws Exception {
        callFailureCountryDrillDetailRoamingService = new CallFailureCountryDrillDetailRoamingService();
        attachDependencies(callFailureCountryDrillDetailRoamingService);
    ...
    private void insertRawData() throws SQLException {
        ...
        insertRowIntoRawTable(timestamp, "1", "438", 1, MCC_FOR_ARGENTINA, SAMPLE_MNC);
        insertRowIntoRawTable(timestamp, "2", "438", 1, MCC_FOR_ARGENTINA, SAMPLE_MNC);
        insertRowIntoRawTable(timestamp, "3", "456", 1, MCC_FOR_ARGENTINA, SAMPLE_MNC);
        insertRowIntoRawTable(timestamp, "3", "456", 1, MCC_FOR_ARGENTINA, SAMPLE_MNC);
    }
    ...
    public void testGetCountryRoamingData_5Minutes() throws Exception {
        ...
        parameters.putSingle(MAX_ROWS, "100");
        final String result = runQuery(callFailureCountryDrillDetailRoamingService, parameters);
    ...
    private void validateResult(final String json) throws Exception {
        ...
        assertThat(result1.getImsi(), anyOf(is("1"), is("2")));
        ...
        assertThat(result2.getImsi(), anyOf(is("1"), is("2")));
