...
/**
 * Desrible RNCCellKPIServiceParamUtil
 * This is a utility class that convert different kinds of format to the format we need to pass to BIS.
 *
 * @author ezhelao
 * @since 11/2011
 */
...
public class RNCCellKPIServiceParamUtil {
    ...
    public List<String> extractCellIDFromURLParam(final String longID) {
        ...
        if (longID.contains(COMMA)) // its from the UI
        {
            ...
            throw new RuntimeException(INVALID_PARAM_ERROR_MSG);
        }
    ...
    public List<String> convertCommaSeparatedStringToList(final String commaSeparatedList) {
        ...
        try {
            ...
            ServicesLogger.warn(this.getClass().getName(), "convertCommaSeparatedStringToList",
                    "invalid param format for CELL or RNC ", ex.getMessage());
        }
     ...
     * @return a KPIDataType object that contain the changes rate to the previous day.It also contains the information
     *         of the current day.
     */
    public KPIDataType getKPIDataTypeFromReturnedData(final NodeQueryKPIDataType day,
            final NodeQueryKPIDataType previousDay) {
        final HashMap<String, String> map = new HashMap<String, String>();
        ...
        if (previousDay.getRequestTotal() == 0) {
            ...
            final Double requestTotalChangeRate = 100 * (double) requestTotalChange
                    / (double) previousDay.getRequestTotal();
