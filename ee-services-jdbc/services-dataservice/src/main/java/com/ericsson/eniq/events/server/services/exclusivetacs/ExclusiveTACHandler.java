...
import static com.ericsson.eniq.events.server.common.ApplicationConstants.GROUP_KEYS;
import static com.ericsson.eniq.events.server.common.ApplicationConstants.GROUP_NAME_PARAM;
import static com.ericsson.eniq.events.server.common.ApplicationConstants.GROUP_TABLE;
...
import static com.ericsson.eniq.events.server.common.ApplicationConstants.URI_PATH_GROUP;

/**
 * Class responsible for some of the business logic around exclusive TAC handling
 * See http://atrclin2.athtem.eei.ericsson.se/wiki/index.php/ENIQ_Events_Services_Design_Rules#Design_Rules_for_Exclusive_TAC_handling
 * for the rules governing this class
 *
 * @author EEMECOY
 */
@Stateless
...
public class ExclusiveTACHandler {
    ...
    public boolean isExclusiveTacGroup(final String groupName) {
        return EXCLUSIVE_TAC_GROUP_NAME.equals(groupName);
    }
    ...
    public boolean isTacInExclusiveTacGroup(final String tac) {
        ...
        final String query = this.templateUtils.getQueryFromTemplate(
                templateMappingEngine.getTemplate(URI_PATH_GROUP, requestParameters, drillType),
    ...
    public boolean queryIsExclusiveTacRelated(final MultivaluedMap<String, String> requestParameters) {
        ...
        return queryIsExclusiveTacRelated(requestParametersWrapper.getGroupName(),
                requestParametersWrapper.getTacParamFromNodeOrTacParam());
    }

    /**
     * In most cases we should exclude all events that are related to a TAC that is in the EXCLUSIVE_TAC group.
     * The exceptions are:
     * 1. It is a Terminal Group event analysis and the group name is EXCLUSIVE_TAC
     * or
     * 2. A TAC is explicitly requested from the UI. This is necessary to cater for drilldowns from point 1.
     *
     * @param requestParameters request parameters passed from UI
     * @boolean true if template should apply tac exclusion, false otherwise
     * @deprecated This logic is used to populate a boolean in the SGEH event analysis templates which controls the flow
     *             of the template logic.  This approach should not be used in velocity templates - there should be no business logic
     *             in templates.  Additionally, this method doesn't look correct, it should consider the case where the TAC parameter
     *             is passed to the services layer with the NODE parameter rather than the TAC parameter.
     */
    @Deprecated
    ...
    /**
     * Check to see if the requestParameters contains a groupname parameter with the value EXCLUSIVE_TAC.
    ...
    private boolean requestContainsExclusiveTacGroup(final MultivaluedMap<String, String> requestParameter) {
        if (requestParameter.containsKey(GROUP_NAME_PARAM)) {
            ...
            if (EXCLUSIVE_TAC_GROUP_NAME.equals(groupNameParam)) {
                return true;
            }
        }
        return false;
