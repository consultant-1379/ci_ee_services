...
public class GroupDataService {
    ...
    public void createGroupCache() {
        ...
        try {
            ...
            ServicesLogger.detailed(getClass().getName(), "createGroupCache", "Group Cache Rebuilt against "
            ...
            try {
                if (dwhrepConn != null) {
                    ServicesLogger.detailed(Level.FINE, getClass().getName(), "createGroupCache", "Closing dwhrepConn "
                            + dwhrepConn);
                    dwhrepConn.close();
                    ServicesLogger.detailed(Level.FINE, getClass().getName(), "createGroupCache", "Is closed: "
                            + dwhrepConn.isClosed());
                }
            } catch (final SQLException e) {
    ...
    /**
     * Get the Events active techpack version.
     * Groups can be specific to a version.
     * @param dwhrep Connection to dwhrep db
     * @return The active versionId for the events tech pack, null if not events tech pack is active.
     * @throws RockException Connectrion errors
    ...
    public Map<String, Group> getGroupsForTemplates() {
        ...
        if (!GroupTypesCache.areGroupsDefined(activeTechpackVersion)) {
            ServicesLogger.warn(getClass().getName(), "getGroupsForTemplates",
                    "No Group Definitions defined in Tech Pack " + activeTechpackVersion);
            return groupMap;
        }
    ...
    private Group getCreateVelocityGroup(final GroupTypeDef aGroupDefinition) {
        ...
        return new Group(aGroupDefinition.getGroupType(), aGroupDefinition.getTableName(),
    ...
    public GroupTypeDef getGroupDefinition(final String groupDefType) {
        ...
        if (!GroupTypesCache.areGroupsDefined(activeTechpackVersion)) {
            ServicesLogger.warn(getClass().getName(), "getGroupDefinition",
                    "No Group Definitions defined in Tech Pack " + activeTechpackVersion);
            return null;
        }
        final String upper = groupDefType.toUpperCase();
        if (!GroupTypesCache.isGroupMgtType(activeTechpackVersion, upper)) {
            ServicesLogger.warn(getClass().getName(), "getGroupDefinition", "No Group Definition called " + upper
                    + " defined in Tech Pack " + activeTechpackVersion);
            return null;
        }
