...
public class EventsTechPackTimerangeQuerierTest extends BaseJMockUnitTest {
    ...
    public void testGetLatestTables_CFA_RAW() {
        ...
        expectCallsOnMockedObjectsForLatestTablesVolumeBasedTechPacks(view,
                EventsTechPackTimerangeQuerier.GET_LATEST_RAW_TABLES, parameters, result, true);
    ...
    public void testGetRAWTablesUsingQuery() {
        ...
        final Map<String, Object> parameters = new HashMap<String, Object>();
        parameters.put(RAW_TIMERANGE_VIEW, view + "_TIMERANGE");
        expectCallsOnMockedObjectsForRawTablesVolumeBasedTechPacks(view, EventsTechPackTimerangeQuerier.GET_RAW_TABLES,
    ...
    public void testGetLatestTablesUsingQuery_DC_Z_ALARM_INFO() {
        ...
        expectCallsOnMockedObjectsForLatestTablesTimeBasedTechPacks(rawTable,
                EventsTechPackTimerangeQuerier.GET_LATEST_RAW_TABLES_NO_TIMERANGE, parameters, result, false);
    ...
    public void testGetRAWTablesUsingQuery_DC_Z_ALARM_INFO() {
        ...
        final Map<String, Object> parameters = new HashMap<String, Object>();
        parameters.put(RAW_TYPE, rawTable);
        expectCallsOnMockedObjectsForRawTablesTimeBasedTechPacks(rawTable,
                EventsTechPackTimerangeQuerier.GET_RAW_TABLES_NO_TIMERANGE, parameters, result, false);
        assertThat(timerangeQuerier.getRAWTablesUsingQuery(mockedFormattedDateTimeRange, rawTable), is(result));
    }

    @SuppressWarnings("unchecked")
    private void expectCallsOnMockedObjectsForLatestTablesVolumeBasedTechPacks(final String view,
            final String pathName, final Map<String, Object> parameters, final List<String> result,
        ...
        mockery.checking(new Expectations() {
            {
                ...
                one(mockedTemplateMappingEngine).getTemplate(with(same(pathName)), with(any(MultivaluedMap.class)),
                        with(same((String) null)));
                ...
                one(mockedDataServiceQueryExecutor).getData(with(same(CANCEL_REQ_NOT_SUPPORTED)), with(same(query)),
                        with(any(Map.class)), with(any(ResultSetTransformer.class)), with(same(loadBalancingPolicy)));
    ...
    private void expectCallsOnMockedObjectsForRawTablesVolumeBasedTechPacks(final String view, final String pathName,
            final Map<String, Object> parameters, final List<String> result, final boolean isTimeRange) {
        final String templateFile = " the template file";
        final LoadBalancingPolicy loadBalancingPolicy = mockery.mock(LoadBalancingPolicy.class);
        final String query = "an sql query";
        mockery.checking(new Expectations() {
        ...
        mockery.checking(new Expectations() {
            {
                ...
                one(mockedTemplateUtils).getQueryFromTemplate(templateFile, parameters);
                will(returnValue(query));
                one(mockedDataServiceQueryExecutor).getDataFromRepdb(with(same(CANCEL_REQ_NOT_SUPPORTED)),
                        with(same(query)), with(any(Map.class)), with(any(ResultSetTransformer.class)));
                will(returnValue(result));
            }
        });
    }
