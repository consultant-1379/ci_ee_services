...
/**
 * Class responsible for creating the TechPackList object, containing information about the
 * raw and aggregation tables applicable for given tech packs and given time ranges and user inputted parameters
 *
 * @author eemecoy
 */
@Stateless
...
public class TechPackListFactory {
    ...
    public boolean shouldQueryUseAggregationView(final String timerange, final AggregationTableInfo aggregationTableInfo) {
        if (EventDataSourceType.AGGREGATED_1MIN.toString().equalsIgnoreCase(timerange)
     ...
     * is to be obsoleted
     */
    private String getSuccessAggregationView(final String time, final String techPackName,
            final AggregationTableInfo aggregationTableInfo) {
        if (techPackName.startsWith(EVENT_E_GSN_DT_TPNAME)) {
     ...
     */
    private String getAggregationView(final String time, final String techPackName,
            final AggregationTableInfo aggregationTableInfo, final String key) {
     ...
     */
    private Map<String, List<String>> getAggregationViewsWithMeasurementTypes(final String time,
            final String techPackName, final AggregationTableInfo aggregationTableInfo,
        ...
        for (final String aggregationViewForQueryType : allAggregationViewsList) {
            ...
            aggregationViewsWithMeasurementTypesMap.put(aggregationViewForQueryType,
                    aggregationViewsWithMeasurementTypesList);
        }
     ...
     * is to be obsoleted
     */
    private String getErrorAggregationView(final String time, final String techPackName,
            final AggregationTableInfo aggregationTableInfo) {
        if (techPackName.startsWith(EVENT_E_GSN_DT_TPNAME)) {
            return getPlainAggregationView(time, techPackName, aggregationTableInfo);
        }
    ...
    /**
     * Determine the raw and aggregation tables to use for this query
     * Note that this method currently fetches the raw tables for every query regardless of whether the query
     * requires the raw tables
     *
     ...
     * @return
     */
    public TechPackList createTechPackListWithSpecifiedAggregation(final List<String> techPacks,
    ...
    public TechPackList createTechPackListWithKeys(final List<String> techPacks, final List<String> rawTableKeys,
        ...
        for (final String techPackName : techPacks) {
            final TechPackRepresentation techPack = new TechPackRepresentation(techPackName);
            if (isTechPackLicensed(techPack.getLicencedName())) {
                final String suc_time = returnSuccessAggregateViewType(
                        dateTimeHelper.getEventDataSourceType(dateTimeRange), techPackName);
                ...
                for (final String rawTableKey : rawTableKeys) {
                    final List<String> rawTables = rawTableFetcher.fetchRawTablesForTechPackAndKey(dateTimeRange,
                            techPackName, rawTableKey);
    ...
    public TechPackList createTechPackListWithMeasuermentType(final List<String> techPacks,
            final List<String> measurementTypes, final FormattedDateTimeRange dateTimeRange,
        ...
        for (final String techPackName : techPacks) {
            final TechPackRepresentation techPack = new TechPackRepresentation(techPackName);
            if (isTechPackLicensed(techPack.getLicencedName())) {
                //@TODO this needs to be removed, only included here to allow temporary usage of multiple tech packs in one service
                //will only work for the smartone tech pack
                if ((techPackName.startsWith(EVENT_E_RAN_SESSION_TPNAME) || techPackName
                        .startsWith(EVENT_E_CORE_SESSION_TPNAME)) && techPacks.size() > 1) {
                    techPack.setPlainAggregationView(getPlainAggregationView(time, techPackName, aggregationTableInfo));
                    techPack.setRawTables(rawTableFetcher.getRawTables(dateTimeRange, techPackName));
                } else {
                    techPack.setAggregationViewsWithMeasurementTypes(getAggregationViewsWithMeasurementTypes(time,
                            techPackName, aggregationTableInfo, measurementTypes, tableSuffixKey));
                    techPack.setRawTablesWithMeausreTypes(rawTableFetcher.getRawTablesWithMeasurementTypesAndKeys(
                            dateTimeRange, techPackName, measurementTypes, tableSuffixKey));
    ...
    private String getPlainAggregationView(final String time, final String techPackName,
            final AggregationTableInfo aggregationTableInfo) {
        if (techPackName.startsWith(EVENT_E_GSN_DT_TPNAME)) {
            return getPlainAggregationViewForGSN(time, techPackName, aggregationTableInfo);
        }
        return getAggregationView(time, techPackName, aggregationTableInfo, EMPTY_STRING);
    }

    /**
     * Method to cater for the fact that GSN tables need special handling. ie, no _1Min tables and also some types are
     * different see typeToTPAggregationTableMapping
     * 
     * @param time The time view, eg _1Min, _15Min _Day _Raw. _1Min will be converted to _15Min
     * @param techPackName The Techpack Name
     * @param aggregationTableInfo The default Aggregation Table Info. Table names will be modified to cater for GSN
     *            tables
     * @return
     */
    private String getPlainAggregationViewForGSN(final String time, final String techPackName,
            final AggregationTableInfo aggregationTableInfo) {
        final String tempTime = time;
        if (ONE_MINUTE_VIEW.equals(time)) {
            //There are no 1Minute aggregation views, so we should have a blank aggregation info.
            return "";
        }
        final AggregationTableInfo tableInfo = new AggregationTableInfo(
                typeToTPAggregationTableMapping.get(aggregationTableInfo.getAggregationView()),
                aggregationTableInfo.getAggregationTimeRanges());
    ...
    public boolean shouldQueryUseAggregationView(final String type, final String timerange,
            final Map<String, AggregationTableInfo> aggregationViews) {
        final AggregationTableInfo aggregationTableInfo = aggregationViews.get(type);
     ...
     * @return the full name of the aggregation view
     */
    private String getAggregationViewAllCalls(final String time, final String techPackName,
            final AggregationTableInfo aggregationTableInfo, final String key) {
