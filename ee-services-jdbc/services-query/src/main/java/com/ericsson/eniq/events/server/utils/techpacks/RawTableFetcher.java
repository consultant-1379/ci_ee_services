...
/**
 * Class responsible for fetching the raw tables to use for a given raw view and time range
 * This class fetches this information from the following sources:
 * <ol>
 * <li> Query the ENIQ engine to fetch the raw tables applicable for the given raw view and time range </li>
 * <li> If this call to the ENIQ engine fails, or if the call returns an empty list of tables, this class will then
 * query the ENIQ engine for the latest tables for this raw view </li>
 * <li> If this call to the ENIQ engine fails, or if the call returns an empty list of tables, this class will
 * run an SQL query on the time range view for the given tech pack.  This SQL query selects the raw tables
 * for the given time range from the appropriate time range view </li>
 * <li> If this SQL query returns no tables, this class will then run an SQL query on the time range view to fetch
 * the latest tables </li>
 * </ol>
 *
 * @author eemecoy
 */
...
public class RawTableFetcher {
    ...
    /**
     * Query for the SUC tables for this tech pack and this time range eg for the EVENT_E_SGEH tech pack, this metho
     * will look for the EVENT_E_SGEH_SUC_RAW tables
     *
    ...
    private List<String> fetchRawTablesForTechPackAndKeyThatHaveRawErrOrSucTables(
            final FormattedDateTimeRange dateTimeRange, final String techPackName, final String typeOfTable) {
    ...
    public List<String> fetchRawTablesForTechPackAndKey(final FormattedDateTimeRange dateTimeRange,
    ...
    /**
     * Query for the ERR tables for this tech pack and this time range eg for the EVENT_E_SGEH tech pack, this method
     * will look for the EVENT_E_SGEH_ERR_RAW tables
     *
     * @param dateTimeRange date time range to query for
     * @param techPackName  the tech pack to query for eg EVENT_E_SGEH
     * @return list of raw tables for this tech pack that are applicable for this time range
     ...
     * @deprecated the methods in this class that take in a dateTimeRange and a techpackName should be used instead
     *             ie the getRawTables(), getRawErrTables() or getRawSucTables() methods
     */
    @Deprecated
    public List<String> getRAWTables(final FormattedDateTimeRange dateTimeRange, final String rawTableType,
            final String templateKeyForRawTablesList) {
    ...
    String[] getViewNamesForTemplateKey(final String rawTableType, final String templateKeyForRawTablesList) {
        ...
        } else if (isGetDTPDPTables(templateKeyForRawTablesList)) {
            viewNames = new String[] { EVENT_E_GSN_DTPDP_RAW };
        } else if (isGetDTTables(templateKeyForRawTablesList)) {
            viewNames = new String[] { EVENT_E_GSN_DT_RAW };
        } else {
    ...
    /**
     * Checks if is get dtpdp tables.
     *
     * @param templateKey the template key
     * @return true, if checks if is get dtpdp tables
     * @deprecated included here for backwards compatiblity only
     */
    @Deprecated
    private boolean isGetDTPDPTables(final String templateKey) {
        return templateKey.equals(RAW_DTPDP_TABLES);
    }
    ...
    /**
     * return true if template key for table name eg rawLteErrTables contains the
     * text lte (not case sensitive) and doesn't contain the text nonlte false
     * otherwise.
     *
     ...
     */
    private List<String> getTablesFromEngine(final FormattedDateTimeRange newDateTimeRange, final String... viewNames)
            throws ParseException {
        return this.rmiEngineUtils.getTableNames(
                new Timestamp(DateTimeRange.getUTCDateTimeWithSeconds(newDateTimeRange.getStartDateTime()).getTime()),
                new Timestamp(DateTimeRange.getUTCDateTimeWithSeconds(newDateTimeRange.getEndDateTime()).getTime()),
                viewNames);
    }
    ...
    /**
     * Query for the (raw, ie neither ERR or SUC) tables for this tech pack and this time range eg for the EVENT_E_RAN_CFA tech pack, this method
     * will look for the EVENT_E_RAN_CFA_RAW tables
     *
     * @param dateTimeRange date time range to query for
     * @param techPackName  the tech pack to query for eg EVENT_E_SGEH
     * @return list of raw tables for this tech pack that are applicable for this time range
    ...
    /**
     * Query for the (raw, ie neither ERR or SUC) tables for this tech pack and this time range with measurement types
     * Eg for the EVENT_E_RAN_HFA tech pack, this method will look for the EVENT_E_RAN_HFA_SOHO_ERR_RAW tables
     *
     * @param dateTimeRange    date time range to query for
     * @param techPackName     the tech pack to query for eg EVENT_E_SGEH
     ...
     * @return list of raw tables for this tech pack that are applicable for this time range
     */
    public List<String> getRawTablesWithMeasurementTypesAndKeys(final FormattedDateTimeRange dateTimeRange,
            final String techPackName, final List<String> measurementTypes, final String rawTableKey) {
