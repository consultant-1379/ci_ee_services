...
import com.ericsson.eniq.events.server.query.QueryParameterTypeMap;
...
public class QueryUtils {
    ...
    /**
     * Alternative to viewAggregationTimeRangeGrid Type safe and more readable.
     * Based on time range indicate the data type source for grid data whether
     * aggregated or raw.
     * 
     * @param timeRange
    ...
    private EventDataSourceType getEventDataSourceType(final long timeRangeInMinutes) throws WebApplicationException {
        ...
        if (DateTimeUtils.oneMinuteAggregationRange(timeRangeInMinutes)
                && applicationConfigManager.getOneMinuteAggregation()) {
            ...
            result = EventDataSourceType.AGGREGATED_15MIN;
                ...
                || (DateTimeUtils.oneMinuteAggregationRange(timeRangeInMinutes) && !applicationConfigManager
    ...
    public Map<String, QueryParameter> mapRequestParameters(final MultivaluedMap<String, String> requestParameters,
        ...
        if (StringUtils.isNotBlank(node)) {
            ...
            requestParameters.putSingle(RAT_PARAM,
                    nodeValuesByType.getAndConvertRatToIntegerValue(ratDescriptionMappingUtils));
        ...
        if (dateTimeRange != null) {
            ...
            /*
             * do UTC conversion, if timeRange >=1week & key=ERR, TOTAL or SUC or DV_DRILL_DOWN
             * also allow UTC conversion of packet switch Subscriber drills as Summary uses DAY tables
             */
            if ((dateTimeRange.getRangeInMinutes() > MINUTES_IN_A_WEEK)
                ...
                startTime = DateTimeUtils.getUTCTime(dateTimeRange.getStartDateTime(),
                        requestParameters.getFirst(TZ_OFFSET), DATE_TIME_FORMAT);
                endTime = DateTimeUtils.getUTCTime(dateTimeRange.getEndDateTime(),
                        requestParameters.getFirst(TZ_OFFSET), DATE_TIME_FORMAT);
            }
    ...
    void addLongToQueryParameters(final MultivaluedMap<String, String> requestParameters,
    ...
    void addLongToQueryParameters(final MultivaluedMap<String, String> requestParameters,
            final Map<String, QueryParameter> queryParameters, final String parameter) {
    ...
    void addStringToQueryParameters(final MultivaluedMap<String, String> requestParameters,
            final Map<String, QueryParameter> queryParameters, final String parameter) {
        final String convertedParameterName = getConvertedParameterName(parameter);
        addStringToQueryParameters(requestParameters, queryParameters, parameter, convertedParameterName);
    }

    void addStringParameterNullIfEmpty(final MultivaluedMap<String, String> requestParameters,
            final Map<String, QueryParameter> queryParameters, final String parameter) {
        if (StringUtils.isNotBlank(requestParameters.getFirst(parameter))) {
            final String parameterValue = requestParameters.getFirst(parameter);
            if (parameterValue.equals(NULL)) {
                queryParameters.put(getConvertedParameterName(parameter),
                        QueryParameter.createNullParameter(java.sql.Types.VARCHAR));
            } else {
                queryParameters.put(getConvertedParameterName(parameter),
                        QueryParameter.createStringParameter(parameterValue));
            }
        }
    }
     ...
     * Maps request parameters to SQL query parameters.
     * 
     * The date/time parameters will be mapped to SQL parameters unless the templatePath is one that doesn't require
     * date/time parameters - see DateTimeWhiteList for more information on this
     * 
     * @param requestParameters
     *          map of URI request parameters
     * @param dateTimeRange
            ...
            void addLong(final String parameter) {
                if (StringUtils.isNotBlank(requestParameters.getFirst(parameter))) {
                    ...
                    try {
                        final Long longParam = Long.valueOf(parameterValue);
                        queryParameters.put(getConvertedParameterName(parameter),
                                QueryParameter.createLongParameter(longParam));
                    } catch (final NumberFormatException e) {
                        add(parameter);
                    }
                }
     ...
     * @return URI paramaters mapped into query parameters
     */
    public Map<String, QueryParameter> mapRequestParametersForHashId(
        ...
        if (dateTimeRange != null) {
            ...
            String startTime = dateTimeRange.getStartDateTime();
            String endTime = dateTimeRange.getEndDateTime();
            // avoid UTC conversion for Circuit Subscriber drills as all requests go to RAW tables
            if (StringUtils.isNotBlank(type)
                    && (type.equals(IMSI_PARAM_UPPER_CASE) || type.equals(MSISDN_PARAM_UPPER_CASE) || type
                            .equals(PTMSI_PARAM_UPPER_CASE))) {
            ...
            if ((dateTimeRange.getRangeInMinutes() > MINUTES_IN_A_WEEK) && StringUtils.isNotBlank(key)
                    && (key.equals(KEY_TYPE_ERR) || key.equals(KEY_TYPE_TOTAL) || key.equals(KEY_TYPE_SUC))
                    && !(isSubscriberType)) {
                startTime = DateTimeUtils.getUTCTime(dateTimeRange.getStartDateTime(),
                        requestParameters.getFirst(TZ_OFFSET), DATE_TIME_FORMAT);
                endTime = DateTimeUtils.getUTCTime(dateTimeRange.getEndDateTime(),
    ...
    private void addLongToRequestParametersIfSet(final MultivaluedMap<String, String> requestParameters,
            final String parameterName, final long parameterValue) {
    ...
    public Map<String, QueryParameter> mapDateParametersForApnRetention(final FormattedDateTimeRange dateTimeRange) {
        final Map<String, QueryParameter> queryParameters = new HashMap<String, QueryParameter>();
        if (dateTimeRange != null) {
            queryParameters.put(DATE_FROM_FOR_APN_RETENTION,
                    QueryParameter.createStringParameter(dateTimeRange.getStartDateTime()));
        }
        return queryParameters;
    }
     ...
     * @return dateTime parameters mapped into query parameters
     */
    public Map<String, QueryParameter> addDateParametersForApnRetention(
            final Map<String, QueryParameter> queryParameters, final FormattedDateTimeRange dateTimeRange) {
        if (dateTimeRange != null) {
            queryParameters.put(DATE_FROM_FOR_APN_RETENTION,
                    QueryParameter.createStringParameter(dateTimeRange.getStartDateTime()));
        }
    ...
    private String getConvertedParameterName(final String parameter) {
        ...
        } else if (parameter.equals(APN_PARAM) || parameter.equals(IMSI_PARAM) || parameter.equals(VENDOR_PARAM)
                || parameter.equals(TAC_PARAM) || parameter.equals(MAN_PARAM) || parameter.equals(PTMSI_PARAM)
                || parameter.equals(MSISDN_PARAM)) {
    ...
    private String getConvertedParameterNameForHashId(final String parameter) {
        String newParameter = null;
        if (parameter.equals(IMSI_PARAM) || parameter.equals(VENDOR_PARAM) || parameter.equals(TAC_PARAM)
                || parameter.equals(MAN_PARAM) || parameter.equals(MSC_PARAM) || parameter.equals(MSISDN_PARAM)) {
            newParameter = parameter.toUpperCase();
    ...
    public String getRATValueAsInteger(final MultivaluedMap<String, String> requestParameters) {
        ...
        // whereas if the RAT is included in the overall NODE parameter, its as the
        // RAT description
        final NodeValuesByType nodeValuesByType = new NodeValuesByType(requestParameters.getFirst(NODE_PARAM),
    ...
    public class NodeValuesByType {
        ...
        /**
         * Gets the RAT - converted into integer value found in the parameter
         * ratDescriptionMappings
         * 
    ...
    /**
     * @param hashUtilities the hashUtilities to set
     */
    ...
    /**
     * Create a QueryParameter object for the supplied parameter name and value
     * The type of the parameter is determined from a static class, QueryParameterTypes
     * 
    ...
    /**
    *
    * @param controller String representation of All Controller information
    *        e.g ONRM_ROOT_MO_R:RNC01:RNC01,Ericsson,3G
    ...
    */
     ...
     * @param bsc The fdn of the controller node.
     * @param hier2 
