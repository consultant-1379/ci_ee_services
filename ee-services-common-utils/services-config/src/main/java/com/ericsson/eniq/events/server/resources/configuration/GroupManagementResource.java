...
public class GroupManagementResource extends AbstractResource {
    ...
    public String editGroup(@QueryParam(QUERY_PARAM_ACTION)
    final String action, final Groupmgt groupmgt) throws InvalidFormatException {
    ...
    @GET
    @Path(JSON_FILE)
    public Response downloadGroupsJSON(@QueryParam(QUERY_PARAM_TYPE)
    final List<String> filters, @Context
    final HttpServletResponse response1) throws IOException {
        ...
        final ContentDisposition contentDisposition = ContentDisposition.type("application").fileName(JSON_FILE_NAME)
                .build();
        return Response.ok(null).header("Content-Disposition", contentDisposition).header("Cache-control", "private")
                .header("Cache-Control", "must-revalidate, post-check=0, pre-check=0")
                .header("Content-Description", "File Transfer").header("Expires", "0").build();
    ...
    public Groupmgt downloadGroupsXML(@QueryParam(QUERY_PARAM_TYPE)
    final List<String> filters) {
        return groupManagementService.downloadSortedGroups(filters);
    }

    /*
    *   the function work in place and convert all group name to lower case
    *   as the database is NOT case sensitive
     */
    private void convertGroupNameToLowerCase(final Groupmgt groupmgt) {
        ...
        while (groupIterator.hasNext()) {
            ...
            if (groupName != null) {
                g.setName(groupName.toLowerCase());
            }
        }
    ...
    /**
     * given a groupmgt object, we iterator over all the elements of keys of groups. Update the rat value
     * from RATDescription to RAT integer
     *
     * @param groupmgt
     */
    ...
    */
    private void validateGroupValue(final Groupmgt groupmgt) throws InvalidFormatException {
        ...
        try {
            try {
                checkGroupElement(groupmgt, ANY_STRING_PATTERN, ANY_STRING_PATTERN, IMPORTED_FILE_KEY_PATTERN,
                        IMPORTED_FILE_KEY_PATTERN);
            } catch (InvalidFormatException ex) {
                throw new InvalidFormatException(INVALID_KEY_ERROR_MSG);
            ...
            try {
                ...
                throw new InvalidFormatException(INVALID_IMSI_ERROR_MSG);
            }

            try {
                checkGroupElement(groupmgt, TYPE_TAC, TYPE_TAC, ANY_STRING_PATTERN, TAC_GROUP_ELEMENT_PATTERN);
            } catch (InvalidFormatException ex) {
                throw new InvalidFormatException(INVALID_TAC_ERROR_MSG);
            }
        } catch (NullPointerException ex) {
            ServicesLogger.error("GroupManagementResource", "validateGroupValue", ex);
            throw new InvalidFormatException(NULL_POINTER_ERROR_MSG);

        } catch (PatternSyntaxException ex) {
            ServicesLogger.error("GroupManagementResource", "validateGroupValue", ex);
            throw new InvalidFormatException(INVALID_PATTERN_ERROR_MSG);
     ...
     */
    private void checkGroupElement(final Groupmgt groupmgt, final String groupTypeFilter, final String keyNameFilter,
            final String keyNameRegex, final String keyValueRegex) throws InvalidFormatException {
        for (Group group : groupmgt.getGroup()) {
