...
public class MssKpiQueryFactoryHelper {
    ...
    /**
     * This method will create an sql query that will get the KPI's for MSS. Decisions about groups, aggregations etc
     ...
     * @param templateParmeters Starting parameters. These are configured outside this class and include selections 
     * made in the GUI.
    ...
    public List<String> getMssKPIQuery(final Map<String, Object> templateParmeters) {
        type = (String) templateParmeters.get(TYPE_PARAM);
    ...
    private String buildMssKpiTable() {
        ...
        final String buildFinalSQLFromMasterTemplate = kpiUtilities.getQueryFromTemplate(MSS_COMPLETE_KPI_VM,
                mssParameters);
    ...
    void populateMssParameters(final Map<String, Object> templateParmeters) throws UnsupportedOperationException {
        ...
        mssParameters.put(USE_TAC_EXCLUSION_BOOLEAN, toExcludeTac);
    ...
    private MssAggregationView getAggregationViewToUseVoice() {
        if (type.equals(TYPE_TAC) || type.equals(TYPE_MAN)) {
            return MssAggregationView.TAC_MANUFACTURER_SUMMARY;
        } else if (type.equals(TYPE_BSC) || type.equals(TYPE_CELL)) {
    ...
    private void populateColumnParameters(final List<String> column, final List<String> columnAlias,
            final QueryHelperData queryHelperData, final TypesOfColumns columnType) {
    ...
    String getColumnAsStringForErrors(final QueryHelperData queryHelperData, final List<MssKPI> allKpis,
    ...
    String getErrorStringForSmsOrLocService(final QueryHelperData queryHelperData, final List<MssKPI> allKpis,
            final TypesOfColumns typeOfView) {
        if (isAggregation) {
