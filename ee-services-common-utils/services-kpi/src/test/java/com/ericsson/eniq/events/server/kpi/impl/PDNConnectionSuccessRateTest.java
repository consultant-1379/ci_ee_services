...
public class PDNConnectionSuccessRateTest extends KPIBaseTestClass {
    ...
    public void testgetKPICalculation() {
        final String expected = "max(isnull((case when isnull(err.EVENT_ID, suc.EVENT_ID)=11 then (cast (round((isnull((isnull(NO_OF_SUCCESSES,0) )/cast(isnull(NO_OF_ERRORS,0) + isnull(NO_OF_SUCCESSES,0) as float),0))*100, 2) as numeric(5,2))) end),0)) as 'PDN Connection Success Rate'";
