...
public class AttachTimeAverageKPITest extends KPIBaseTestClass {
    ...
    public void testgetKPICalculation() {
        final String expected = "max(isnull((case when isnull(suc.EVENT_ID, err.EVENT_ID)=5 then (cast (round((isnull((isnull(TOTAL_DURATION,0) )/cast(isnull(NO_OF_SUCCESSES,0) as float),0)), 2) as numeric(5,2))) end),0)) as 'Attach Time Average'";
