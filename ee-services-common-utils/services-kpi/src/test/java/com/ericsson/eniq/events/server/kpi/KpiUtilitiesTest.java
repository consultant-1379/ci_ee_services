...
public class KpiUtilitiesTest extends KPIBaseTestClass {
    ...
    /**
     * Making sure that if we don't supply any parameters that no parameter replacement is going on. Probably need to
     * to this for every template to make sure something stupid isn't being done in the code
     */
    @Test
    public void testGettingTemplateNoParameters() {
        ...
        final String expected = "max(isnull((case when isnull(${error_table_alias}.EVENT_ID, ${success_table_alias}.EVENT_ID)=$event_id_value then (cast (round((isnull((isnull($no_of_successes,0) )/cast(isnull($no_of_errors,0) + isnull($no_of_successes,0) as float),0))*100, 2) as numeric(5,2))) end),0)) as '$kpi_name'";

        Assert.assertThat(sqlFragment, Matchers.equalToIgnoringWhiteSpace(expected));
    }

    /**
     * Making sure that when we try to get the sql from template with correct parameters, that we get the expected 
     * result. Probably need to do this for every template.
     */
    @Test
    public void testGettingTemplate() {
        ...
        final String expected = "max(isnull((case when isnull(err.EVENT_ID, suc.EVENT_ID)=$event_id_value then (cast (round((isnull((isnull(NO_OF_SUCCESSES,0) )/cast(isnull(NO_OF_ERRORS,0) + isnull(NO_OF_SUCCESSES,0) as float),0))*100, 2) as numeric(5,2))) end),0)) as 'kpi_test'";
     ...
     * Will need to add something similar for event subtype id's
     */
