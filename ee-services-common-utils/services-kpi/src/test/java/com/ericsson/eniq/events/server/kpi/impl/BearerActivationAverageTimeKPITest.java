...
public class BearerActivationAverageTimeKPITest extends KPIBaseTestClass {
    ...
    public void testgetKPICalculationRaw() {
        final String expected = "max(isnull((case when isnull(suc.EVENT_ID, err.EVENT_ID)=10 then (cast (round((isnull((isnull(TOTAL_DURATION,0) )/cast(isnull(NO_OF_SUCCESSES,0) as float),0)), 2) as numeric(5,2))) end),0)) as 'Bearer Activation Time Average'";
        final Map<String, Object> parameters = new HashMap<String, Object>();
